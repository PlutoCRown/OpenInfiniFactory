"""生成石英场景方块（大理石贴图 + quartz / quartz_pillar / quartz_slope GLB）。

用法：
  python3 scripts/scene_blocks/generate_quartz.py
图标：
...
def make_marble_texture(size=32):
    ...
    for _ in range(5):
        ...
        angle = rng.uniform(-0.6, 0.6) + (
            math.pi * 0.15 if rng.random() < 0.5 else -0.2
        )
...
for name, mesh_fn in [
    ("quartz", cube_mesh),
    ("quartz_slope", slope_mesh),
    ("quartz_pillar", pillar_mesh),
]:
...
def check(path):
    ...
    (clen,) = S.unpack_from("<I", data, 12)
    j = json.loads(data[20 : 20 + clen].decode().rstrip(" "))
    bin_off = 20 + clen
    (blen,) = S.unpack_from("<I", data, bin_off)
    ...
    def acc(i):
        ...
        for k in range(a["count"]):
            if ctype == 5126:
                out.append(S.unpack_from("<" + "f" * n, blob, off + k * 4 * n))
            else:
                out.append(S.unpack_from("<" + "H" * n, blob, off + k * 2 * n))
        return out

    pos, nor, idx = acc(0), acc(1), [x[0] for x in acc(3)]
    bad = 0
    for t in range(len(idx) // 3):
        i0, i1, i2 = idx[t * 3 : (t + 1) * 3]
        ...
        sn = nor[i0]
        dot = (cx / L) * sn[0] + (cy / L) * sn[1] + (cz / L) * sn[2]
        if dot < 0.5:
            bad += 1
