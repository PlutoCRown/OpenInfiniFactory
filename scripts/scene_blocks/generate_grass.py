"""生成草方块 atlas 贴图与 grass/model.glb（锯齿侧面）。

用法：
  python3 scripts/scene_blocks/generate_grass.py
图标：
...
_png_buf = BytesIO()
atlas.save(_png_buf, format="PNG", optimize=True)
...
def build_cube():
    ...
    for fi, (corners, n) in enumerate(faces):
        ...
        for i, p in enumerate(corners):
            pos.append(p)
            nor.append(n)
            uvs.append(cube_faces_uv(fi, local[i]))
...
def bv(name, t):
    o, l = offsets[name]
    return {"buffer": 0, "byteOffset": o, "byteLength": l, "target": t}
...
bc = bin_blob + b"\x00" * bp
total = 12 + 8 + len(jc) + 8 + len(bc)
out = bytearray()
out += struct.pack("<4sII", b"glTF", 2, total)
out += struct.pack("<I4s", len(jc), b"JSON")
out += jc
out += struct.pack("<I4s", len(bc), b"BIN\x00")
out += bc
