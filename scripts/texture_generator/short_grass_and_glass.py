"""生成 short_grass（交叉面片）与 glass（透明边框立方体）"""
...
def write_glb(path, name, positions, normals, uvs, indices, png, *,
              roughness=0.9, metallic=0.0, alpha_mode='OPAQUE', alpha_cutoff=0.5, double_sided=False):
...
def make_grass_plant_tex(size=16):
    ...
    for blade in range(7):
        ...
        green = (46 + rng.randint(0, 40), 120 + rng.randint(0, 50), 40 + rng.randint(0, 30), 255)
        dark = (30 + rng.randint(0, 20), 80 + rng.randint(0, 30), 25 + rng.randint(0, 20), 255)
        x = float(x0)
...
def short_grass_mesh():
    ...
    add_quad(positions, normals, uvs, indices,
             [(-h,-h,0),(h,-h,0),(h,h,0),(-h,h,0)], (0,0,1), uv)
    # plane in ZY (facing +X)
    add_quad(positions, normals, uvs, indices,
             [(0,-h,h),(0,-h,-h),(0,h,-h),(0,h,h)], (1,0,0), uv)
...
def make_glass_tex(size=16, border=2):
    ...
    for y in range(size):
        for x in range(size):
            on_edge = x < border or y < border or x >= size - border or y >= size - border
            if not on_edge:
                continue
    ...
    if border >= 1:
        for i in range(border, size - border):
            for (x, y) in ((border, i), (size - border - 1, i), (i, border), (i, size - border - 1)):
                if px[x, y][3] == 0:
...
write_glb(ROOT/'short_grass'/'model.glb', 'short_grass', pos, nor, uv, idx, png,
          roughness=0.95, alpha_mode='MASK', alpha_cutoff=0.1, double_sided=True)
write_meta(ROOT/'short_grass'/'meta.json', 'short_grass',
           [False]*6, collision=False)

# glass
pos, nor, uv, idx = cube_mesh()
png = write_png(make_glass_tex(16, border=2))
write_glb(ROOT/'glass'/'model.glb', 'glass', pos, nor, uv, idx, png,
          roughness=0.05, metallic=0.0, alpha_mode='BLEND', double_sided=False)
