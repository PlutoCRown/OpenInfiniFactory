...
/// 交互反馈（放置/删除/按钮等）相对音效音量的倍率；2D 直达耳朵后比空间音更响，压到约 1/3。
const UI_FEEDBACK_GAIN: f32 = 1.0 / 3.0;
/// 同时播放的机器底噪上限（取距玩家最近的若干个）。
const MAX_MACHINE_AUDIO_LOOPS: usize = 10;
...
struct SoundAssets {
    ...
    selection_tick: Handle<AudioSource>,
...
impl FromWorld for SoundAssets {
    fn from_world(world: &mut World) -> Self {
        ...
        Self {
            ...
            selection_tick: asset_server.load("audio/selection_tick.wav"),
...
impl SoundAssets {
    fn source(&self, sound: SoundId) -> Handle<AudioSource> {
        match sound {
            ...
            SoundId::SelectionTick => self.selection_tick.clone(),
...
fn play_sound_events(
    ...
    for event in events.read() {
        ...
        if let (false, Some(position)) = (ui_feedback, event.position) {
...
/// 让传送带和钻头只在模拟运行且通电时拥有持续底噪，且只保留距玩家最近的若干个。
    ...
    let mut ranked: Vec<(f32, IVec3)> = if simulation.running {
        world
            ...
            .map(|pos| {
                ...
                let distance = listener_position
                    .map(|listener| listener.distance(world_pos))
                    .unwrap_or(0.0);
                (distance, pos)
            })
            .filter(|(distance, _)| *distance <= MAX_SOUND_DISTANCE)
            .collect()
    } else {
        Vec::new()
    };
    ranked.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap_or(std::cmp::Ordering::Equal));
    if ranked.len() > MAX_MACHINE_AUDIO_LOOPS {
        ranked.truncate(MAX_MACHINE_AUDIO_LOOPS);
    }
    let desired: HashSet<IVec3> = ranked.into_iter().map(|(_, pos)| pos).collect();

    for (entity, audio_loop) in &loops {
        if !desired.contains(&audio_loop.pos) {
            commands.entity(entity).despawn();
        }
    }

    let existing: HashSet<IVec3> = loops.iter().map(|(_, audio_loop)| audio_loop.pos).collect();
    for pos in desired.difference(&existing) {
        commands.spawn((
