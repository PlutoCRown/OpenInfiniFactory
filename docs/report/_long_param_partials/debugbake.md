...
|------|--------|------|------|------------|-------------------|------|--------|
| `handle_embedded_debug_command` | 15 | helper | **1**（已落地）+ **3 accept** 余量 | `DebugHttpSessionSnapshot`, `DebugHttpPerfSnapshot`, `PlayingWorldParams`；（Run/RunOneTurn 门控字段与 **C7** 同形，**不**整函数挂 C7） | 会话/perf/世界已聚合；余下 `simulation`/`presentation`/`pending_generated`/`signal_cache`/`turn_cache`/`worker`/`sim_log`/`placement`/`player`/`render_ready`/`command` 为命令分发期参数。仅 1 调用方（`poll_debug_http`），禁止再拆空 helper。Place 走 `place_block`+`refresh_world_after_edit(playing)`，**非** C2/C5 | 勿把 HTTP 命令总线改成手段 2（须同步 `String` 回包）。勿发明 `DebugHttpCmdCtx`；勿把余量硬塞进 C3（字段集≠`SimulationTickDeps`） | **P2** |
