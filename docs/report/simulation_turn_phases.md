...
| 2 运动标记 | 重力 + 通电设备打 `StructureMove`，再 merge | 否（规划时脆弱格视为可让出） |
| 3a 脆弱碎裂 | 按计划移除冲突脆弱材料（活塞头压碎 / 挡路 / 结构内撞实心）；与钻头激光销毁分离 | 是（仅碎裂） |
| 3b 执行运动 | 统一执行移动；推杆状态；重生静态 marker | 是（仅位姿 / marker） |
| 4 结构后处理 | 销毁、传送、转换、验收、生成、焊接、滚刷漆、印花附着 | 是（拓扑与材料） |

## 阶段 4 内部顺序

销毁（钻头 + 通电激光）→ 传送 → 转换 → 验收 → 生成 → 焊 → 滚刷漆 → 印花附着 → `refresh_material_structures` → 回合末信号刷新。
...
- 活塞头伸入脆弱格：头成功伸出；脆弱碎；**不**推动该脆弱所在结构
- 传送带 / 平移 / 重力：结构欲进入外部脆弱格 → 碎该脆弱后继续；结构内脆弱撞实心 → 碎该成员，其余继续
- 非脆弱仍正常阻挡
