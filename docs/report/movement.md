...
活塞 / 阻拦器的设备扫描会先按坐标稳定排序。伸出动作在生成移动计划前会声明自身正前方的活塞头目标格；同一回合中同一格只能被一个伸出动作声明，且如果该格已经有未收回的硬头部占用，后续伸入会被跳过。被跳过的设备不会修改 `PusherState`，因此不会出现“状态认为已伸出但头部目标格被别的活塞占用”的不同步情况。
...
### 第三层：传送带目标与 source 坐标

如果两个移动都是传送带，并且前两层也相同，会比较 `ConveyorPriority`：
...
也就是先按传送带移动后的主目标格排序，再按移动方向和 source 坐标排序。多个传送带把结构推向同一个目标位置时，结果由这个稳定键决定，而不是依赖 HashMap 遍历顺序。
...
- 出口平台层必须可放置材料，否则传送无效且不改变世界状态。
- 传送会移动该方块自身的面标记，但不会保留任何材料焊接关系。
- 如果入口方块来自焊接结构，传送后原结构会断开；多个方块分别传过去后，在出口侧也不会自动焊接。
