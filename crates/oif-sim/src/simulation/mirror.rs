...
mod tests {
    ...
    fn mirror_reflects_using_model_geometry() {
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::Mirror, Facing::North, IVec3::X),
            vec![IVec3::NEG_Z]
        );
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::Mirror, Facing::East, IVec3::X),
            vec![IVec3::Z]
        );
    }
    ...
    fn vertical_mirror_reflects_using_local_x_axis() {
        ...
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::VerticalMirror, Facing::North, IVec3::Y),
            vec![IVec3::NEG_X]
        );
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::VerticalMirror, Facing::North, IVec3::X),
            vec![IVec3::NEG_Y]
        );
        // East：局部 +X 变为 -Z；南向激光 +Z 应反射为 +Y
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::VerticalMirror, Facing::East, IVec3::Z),
            vec![IVec3::Y]
        );
    }
    ...
    fn splitter_splits_to_the_other_two_axes() {
        ...
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::Splitter, Facing::North, IVec3::Z),
            vec![IVec3::NEG_X, IVec3::Y]
        );
        // 朝东 +Z 入射 → +Y 与 +X
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::Splitter, Facing::East, IVec3::Z),
            vec![IVec3::Y, IVec3::X]
        );
        // 反向入射时两路出射一并取反
        assert_eq!(
            reflect_laser(LaserOpticsBehavior::Splitter, Facing::North, IVec3::NEG_Z),
